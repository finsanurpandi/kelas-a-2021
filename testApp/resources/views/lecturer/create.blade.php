            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                ...
                @if(request()->routeIs('lecturer.create'))
                    ...
                    <form method="post" action="{{ route('lecturer.store') }}" class="mt-6 space-y-6">
                        ...
                        <div>
                            ...
                            <x-text-input id="nama" name="nama" type="text" class="mt-1 block w-1/2" :value="old('nama')" required autofocus autocomplete="nama" />
                ...
                @else
                    <form method="post" action="{{ route('lecturer.update', $lecturer->nidn) }}" class="mt-6 space-y-6">
                        ...
                        <div>
                            <x-input-label for="nama" :value="__('NAMA')" />
                            <x-text-input id="nama" name="nama" type="text" class="mt-1 block w-1/2" :value="old('nama', $lecturer->nama)" required autofocus autocomplete="nama" />
                            <x-input-error class="mt-2" :messages="$errors->get('nama')" />
                        </div>
                        <div>
                            <x-input-label for="department" :value="__('DEPARTMENT')" />
